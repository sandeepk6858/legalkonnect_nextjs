...
const CourtReporters = async ({searchParams}) => {
  ...
  return (
    <>
          ...
          <div className="flex flex-wrap lg:flex-nowrap gap-[20px] xl:gap-[30px]">
            {data?.data?.users ?
              data?.data?.users?.map((item) => (
                <UserComponent key={item.id} data={item} path="court-reporters" />
              ))
              :
      ...
      {data?.data?.pagination?.last_page > 1 && (
        <PaginationUi pagination={data.data.pagination} searchParams={searchParams} pathname={'/court-reporters'} />
      )}
    </>
  );
};

export default CourtReporters;
