...
const Processservers = async({searchParams}) => {
  ...
  return (
    <>
        ...
        <div className="px-[15px] lg:px-[40px] py-[60px] md:py-[80px] mx-auto ">
          ...
          {data?.data?.users ?
              data?.data?.users?.map((item) => (
                <UserComponent key={item.id} data={item} path='process-servers'/>
              ))
              :
      ...
      {data?.data?.pagination?.last_page > 1 && (
        <PaginationUi pagination={data.data.pagination} searchParams={searchParams} pathname={'/process-servers'} />
      )}
    </>
  );
};
